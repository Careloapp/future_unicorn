...
/** Map a raw call_log DB row to a display-friendly shape. */
function mapLog(raw: any, index: number): DisplayLog {
  const hasConversation = Array.isArray(raw.conversation) && raw.conversation.length > 0;
  const type: CallType = hasConversation ? "Inbound" : "Missed";
  ...
  return {
    ...
    caller: raw.phone_number_encrypted ?? "Unknown",
    number: raw.phone_number_encrypted ?? "—",
    type,
    duration,
...
const CallLogs = () => {
  ...
  useEffect(() => {
    api
      .get<any[]>("/api/call-logs")
      .then((data) => {
        setLogs(data.map(mapLog));
        setLoading(false);
      })
      .catch((err) => {
        console.error("[CallLogs]", err);
        setError("Could not load call logs. Is the backend running?");
        setLoading(false);
      });
  }, []);
        ...
        >
          {loading ? (
            ...
            <div className="py-16 text-center">
              ...
              <p className="font-body text-sm text-gray-500">Call logs will appear here once calls are received.</p>
            </div>
          ) : (
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/5">
                  {["Caller", "Number", "Type", "Duration", "Time"].map((h) => (
                    <th
                      key={h}
              ...
              <tbody>
                {logs.map((log, i) => {
                  ...
                  return (
                    <motion.tr
                      key={log.id}
                      className="border-b border-white/5 last:border-0 hover:bg-white/3 transition-colors cursor-default"
